...
def get_thumbnail_url(url):
    """Fetch the thumbnail URL via microlink.io for a given TikTok URL."""
...
for root, dirs, files in os.walk(gallery_dir):
    for filename in files:
        ...
        # Match youtube_id that has tiktok.com
        match = re.search(
            r"^youtube_id:\s*'?((?:https?://)?(?:www\.)?tiktok\.com/[^\s']+)'?",
            content,
            re.MULTILINE,
        )
        if not match:
        ...
        if thumb_match:
            ...
            if thumb_val and thumb_val not in ("''", '""', "") and "tiktokcdn" not in thumb_val:
