...
for root, dirs, files in os.walk(gallery_dir):
    for filename in files:
        ...
        # Match youtube_id that has twitter or x.com
        match = re.search(r"^youtube_id:\s*'?((?:https?://)?(?:www\.)?(?:twitter\.com|x\.com)[^\s']+)'?", content, re.MULTILINE)
        if match:
            ...
            print(f"Fetching for {filename} -> {tweet_url}")
            image_url = get_twitter_thumbnail(tweet_url)
            if image_url:
